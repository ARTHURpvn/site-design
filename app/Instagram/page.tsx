...
export default function Instagram() {
    ...
    const rendPostView = (post: string) => {
        setClickedPost(post)
        setPostView(true)
    }
    ...
    const rendPosts = () => {
        return posts.map((post, index) => {
            return (
                <div onClick={() => rendPostView(post)} className="w-[240px] h-[240px] max-[480px]:w-[120px] max-[480px]:h-[120px]" key={index}>
                    <Image
                        src={"/" + post}
                        width={240}
                        height={240}
    ...
    return (
        <>
            { postView && (
                <div className="z-50 w-[70%] flex items-center justify-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                    <img 
                        src={"/" + clickedPost}
            ...
            <div className={`w-screen h-screen flex flex-col items-center overflow-x-hidden justify-between ${postView && "blur-md brightness-[.3] overflow-y-hidden"}`} onClick={closePostView}>
